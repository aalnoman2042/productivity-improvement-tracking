...
- **Admin** (`/admin`) — for the accounts named in `ADMIN_EMAILS` (falling
  back to the list in `lib/admin.ts`); a 404 for everyone else. Every account
  with its tracker and logged-day counts. Counts only — no account's data is
